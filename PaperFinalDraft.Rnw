...
p <- ggplot(aes(x=year, y=percent), data=popData)
  ...
  + geom_line()
  + scale_x_continuous(breaks=c(2008, 2008.5, 2009, 2009.5, 2010), 
...
baseplot <- ggplot(mapping = aes(x=citystate, y=Freq, fill = Response, order=Response))
  ...
  + scale_fill_manual(breaks=c("Not at all satisfied", "2", "3", "4", "Extremely satisfied"), 
                      values=colorsB, name="Response") 
  + geom_bar(data = trial2$pos, stat = "identity") 
  + coord_flip() 
  + ggtitle("Community satisfaction") 
  + xlab("") 
  + ylab("")
  + scale_y_continuous(limits=c(-0.5, 1), breaks=seq(from=-0.5, to=0.75, by=0.25), 
...
baseplot <- ggplot(dfcitiesYes) 
  ...
  + coord_flip()
  + guides(fill=FALSE)
  + scale_y_continuous(breaks=seq(from=-.5, to=.5, by=.125),
                       labels=c("50%","", "25%","", "0","", "25%","", "50%"))
  +scale_x_discrete(labels=LETTERS[10:1])
  + ylab("") 
  + xlab("")
  + theme(axis.text.y=element_text(size=12), title=element_text(size=18), 
          axis.text.x=element_text(size=14), strip.text=element_text(size=16))
baseplot
@
...
raceOver <- ggplot(races) 
  + aes(race, n) 
  + geom_bar(stat="identity") 
  + facet_wrap(~year,nrow=3) 
  + ylab("Number of respondents")
  + xlab("")
  + theme(axis.text.y=element_text(size=20), title=element_text(size=24), 
          axis.text.x=element_text(size=18), strip.text=element_text(size=25))
...
baseplot <- ggplot() 
  + aes(InGroup, Freq, fill = Response, order=Response)
  + facet_wrap(~year, nrow=3)
...
# TRUE means minority, and in 2008 minorities were under-rating
  ...
  + theme(legend.text=element_text(size=14), legend.title=element_text(size=16), 
          axis.text.y=element_text(size=16), title=element_text(size=16), 
...
baseplot <- ggplot() 
  ...
  + scale_fill_manual(values=colorsA, breaks=c("Very bad", "2", "3", "4", "Very good"),name="Response")
  + coord_flip()
...
al <- ggplot(newDF09)
  ...
  + geom_text(data = newDF09[newDF09$citystate %in% c("Duluth, MN", "Gary, IN", "Grand Forks, ND", "State College, PA"),],
              aes(y=white, x=nonwhite, label = citystate), vjust=c(-0.6, -0.4, 1.3, -0.6), hjust=0, size=4) 
...
baseplot <- ggplot(mapping = aes(InGroup, Freq, fill = Response, order=Response))
  ...
  + ylab("")
  + theme(legend.text=element_text(size=14), legend.title=element_text(size=16), 
          axis.text.y=element_text(size=12), title=element_text(size=16), 
          axis.text.x=element_text(size=12), strip.text=element_text(size=14))
...
sr <- ggplot(totPos)
  ...
  + geom_text(data = totPos[totPos$citystate %in% c("Wichita, KS", "Gary, IN", "Bradenton, FL"),], 
              aes(y=Nonseniors, x=Seniors, label = citystate), vjust=c(-0.4,-0.4,1), hjust=c(1,0,0), size=4) 
  + xlab("Senior rating") + ylab("Non-senior rating") 
sr
@
...
kd <- ggplot(comp09) 
  ...
  + geom_abline(intercept=0, slope=1) + geom_point(aes(y=OutGroup, x=InGroup)) 
  + geom_text(data = comp09[comp09$citystate %in% c("Grand Forks, ND", "Gary, IN", "St. Paul, MN"),], 
              aes(y=OutGroup, x=InGroup, label = citystate), vjust=c(-0.4,-0.4,1), hjust=c(0,1,0), size=4) 
