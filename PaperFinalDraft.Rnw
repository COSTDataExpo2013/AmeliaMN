...
<<startingoverall>>=
dfcities09 <- citystateResp(sotc09) %>% 
  mutate(year="2009")
dfcities08 <- citystateResp(sotc08) %>% 
  mutate(year="2008")
dfcities10 <- dfcities10 %>% 
  mutate(year="2010")
...
minoritiesYN09 <- sotc09 %>% 
  ...
  filter(is.na(minorities)==FALSE) %>% group_by(citystate, race != "White", minorities) %>% 
  summarise(n = n()) %>% 
  mutate(frac=prop.table(n), year=2009)
...
<<kidsbyYear>>=
kidGroup10 <- sotc10 %>% 
  filter(children == "Yes" | children == "No", is.na(families_kids) == FALSE) %>% 
...
kidGroup08 <- sotc08 %>% 
  filter(children == "Yes" | children == "No", is.na(families_kids) == FALSE & families_kids != "(DK)" & families_kids != "(Refused)") %>% 
  group_by(citystate, children, families_kids) %>% 
  summarise(n = n()) %>% 
  mutate(frac=prop.table(n), year=2008)
names(kidGroup08) <- n1
