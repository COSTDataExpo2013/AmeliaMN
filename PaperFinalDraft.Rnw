...
missing08 <- count_na(sotc08$recommend)
missing <- c(missing08, missing09, missing10)
...
<<OverallSeniors>>=
seniorGroup10 <- filter(sotc10, is.na(seniors) == FALSE) %>% group_by(citystate, age >= 62, seniors) %>% summarise(n = n()) %>% mutate(frac=prop.table(n), year=2010)

names(seniorGroup10) <- n1

seniorGroup09 <- filter(sotc09, is.na(age) == FALSE) %>% group_by(citystate, age >= 62, seniors) %>% summarise(n = n()) %>% mutate(frac=prop.table(n), year=2009)
...
<<kidsbyYear>>=
kidGroup10 <- filter(sotc10, children == "Yes" | children == "No", is.na(families_kids) == FALSE) %>% group_by(citystate, children, families_kids) %>% summarise(n = n()) %>% mutate(frac=prop.table(n), year=2010)
names(kidGroup10) <- n1

kidGroup09 <- filter(sotc09, children == "Yes" | children == "No", is.na(families_kids) == FALSE) %>% group_by(citystate, children, families_kids) %>% summarise(n = n()) %>% mutate(frac=prop.table(n), year=2009)
names(kidGroup09) <- n1
...
kidGroup08 <- filter(sotc08, children == "Yes" | children == "No", is.na(families_kids) == FALSE & families_kids != "(DK)" & families_kids != "(Refused)") %>% group_by(citystate, children, families_kids) %>% summarise(n = n()) %>% mutate(frac=prop.table(n), year=2008)
names(kidGroup08) <- n1
